...
def apply_offset_attn_delta(model, base_weights, delta, batch_idx, max_update=8192, kernel_size=5):
    """
    Applies a partial update to the model weights using an offset window and 1/sqrt attenuation.
    
    Args:
        model: The model to update.
        base_weights: Flattened weights before update (1D tensor).
        delta: Predicted delta from hypertransformer (1D tensor, length = max_update).
        batch_idx: Current batch index (for sliding offset).
        max_update: Number of weights to update per batch.
        kernel_size: How far to attenuate the delta in both directions.
    """
    ...
    start = max(0, offset - kernel_size)
    end = min(flat_dim, offset + max_update + kernel_size)

    # Create 1/sqrt attenuation weights
    attn_weights = torch.tensor([
        1 / math.sqrt(abs(i - (offset + j % max_update)) + 1)
        for j in range(max_update)
        for i in range(start, end)
    ], dtype=delta.dtype).view(max_update, end - start)

    # Normalize attenuation across dim 1
    attn_weights = attn_weights / attn_weights.sum(dim=1, keepdim=True)

    # Apply attenuated update
    for j in range(max_update):
        for i in range(start, end):
            updated[i] += delta[j] * attn_weights[j, i - start]

    # Load updated weights back into the model
...
def main():
   ...
   #model = Transformer_Model(vocab_size, d_model, n_layers, n_heads, seq_len).to(device)
    ...
    hyper_model = HyperTransformer(input_dim=seq_len*2, model_dim=d_model, num_layers=2, num_heads=n_heads, weight_dim=flat_dim).to(device)
